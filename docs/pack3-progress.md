...
- [x] T-07 Driver handover (Bàn giao lại) (mig 047 — driver_handovers + _failed_orders + _items + confirm_driver_handover RPC: atomic restock via stock_entries+stock_entry_lines+batches+fifo_layers, flips failed orders to status=cancelled / current_workflow_stage=delivery_failed, auto-closes workflow_session for the delivery. mig 051 adds swap_movement_id FK on items + bumps swap_stock_movements.qty_returned_in_base_uom on confirm. lib/handover/confirm.ts wrapper. Page /deliveries/[id]/handover loads failed-order lines + unused swap stock (Q7) into the same items grid; "SWAP" badge + "Đã đổi cho khách rồi" checkbox flips destination_zone (D7).)
- [x] T-08 Phiếu thu lái xe TT200 (component PaymentReceiptTT200 + numberToVietnameseWords helper). Wired on /finance/cash-receipts/[id]: separate "In phiếu thu (TT200)" button toggles html[data-print-mode='receipt-tt200'], globals.css renders only .print-receipt-tt200-only block. Pulls organization name + collector name + receipt code/date/amount automatically.
...
- [x] T-12 Phiếu xuất hàng đem đổi (mig 049 — swap_stock_movements table linked to stock_entries, tracks qty + qty_returned + fifo_consumptions snapshot for re-stocking. inventory/stock-out picking page: dashed amber section "Hàng đem đi đổi (dự phòng)" with + Thêm SP dialog backed by lazy-loaded product catalog, qty/reason editable rows. Save flow appends [Swap]-tagged stock_entry_lines + swap_stock_movements rows. inventory/entries/[id] print slip splits "HÀNG THEO ĐƠN" vs dashed-bordered "HÀNG ĐEM ĐI ĐỔI" sections. T-07 handover loads remaining unused swap rows automatically.)

## Sprint 4 — Permission + payroll
- [x] T-13 Phân quyền per-user override (mig 041 — table + resolver fn. /settings/users/[id]/permissions page: tri-state matrix per (module × action) with "Theo vai trò / Cấp quyền / Thu hồi" Select per cell; effective value badge next to it, Reset toàn bộ button, header shortcut from /settings/users/[id]).
...
- [x] T-15 KPI tiers + Order-count bonus + Activity bonus (mig 043 — 3 per-user tables; lib/payroll/bonus.ts compute helpers. /settings/users/[id]/salary page: 3 sections (KPI tiers per month with min_revenue × percent/fixed, order-count configs with effective range, activity bonus monthly upsert), each with inline add + delete; header shortcut from /settings/users/[id]).
- [x] T-16 Bảng lương (mig 050 — payroll_runs + payroll_run_items + compute_payroll_run() + lock_payroll_run() RPCs + enforce_payroll_lock trigger. lib/payroll/run.ts wraps RPCs + setManualAdjustment. /hr/payroll/runs page lists kỳ + active run details with totals tiles + per-user grid showing prorated base + KPI + order-count bonus + activity bonus + BHXH (10.5% flat) + manual_adjustment. Lock flips status='locked', trigger blocks further mutations. "Xuất Excel" button generates xlsx via shared downloadXlsx helper with all columns + grand total row.)
