...
export function SourceInvoiceModal({
  ...
  return (
    <>
      <button type="button" aria-label="Đóng" onClick={onClose} className="fixed inset-0 z-40 cursor-default bg-[#0f172a]/40" />
      <div
        role="dialog"
      ...
      >
        <div className="flex h-14 shrink-0 items-center justify-between border-b border-[#e2e8f0] px-5">
          <h2 className="text-[16px] font-bold text-[#0f172a]">Chọn hóa đơn gốc để trả hàng</h2>
          <button type="button" aria-label="Đóng" onClick={onClose} className="h-8 w-8 rounded-lg text-[18px] leading-none text-[#64748b] hover:bg-[#f1f5f9]">×</button>
        </div>

        <div className="flex min-h-0 flex-grow">
          <aside className="w-[286px] shrink-0 overflow-y-auto border-r border-[#e2e8f0] p-4">
            <p className="mb-1.5 text-[10.5px] font-bold uppercase tracking-[0.06em] text-[#64748b]">Tìm theo</p>
            <input
              type="text"
              ...
              className="h-9 w-full rounded-lg border border-[#cbd5e1] px-2.5 text-[12.5px]"
            />

            <p className="mb-1.5 mt-4 text-[10.5px] font-bold uppercase tracking-[0.06em] text-[#64748b]">Lọc</p>
            <label className="flex items-start gap-2 py-1.5 text-[12.5px] text-[#334155]">
              <input
                type="checkbox"
                checked={chiKhachNay}
              ...
              <span>
                ...
                {!customerId && (
                  ...
                  <span className="mt-px block text-[11px] text-[#94a3b8]">
                    chưa chọn khách nên chưa lọc được
                  </span>
                )}
              </span>
            </label>
            <label className="flex items-center gap-2 py-1.5 text-[12.5px] text-[#334155]">
              <input
                type="checkbox"
                checked={anDaTraHet}
          ...
          <div className="flex min-h-0 flex-grow flex-col">
            ...
            <div className="min-h-0 flex-grow overflow-y-auto">
              {loi && <p className="px-4 py-4 text-[13px] font-semibold text-[#dc2626]">Không tải được — {loi}</p>}
              {!loi && dangTai && <p className="px-4 py-10 text-center text-[13px] text-[#64748b]">Đang tải…</p>}
              ...
              {ketQua.map(({ r, conTra }, i) => (
                <div
                  key={r.id}
                  className={`grid min-h-[48px] items-center border-b border-[#f1f5f9] px-4 ${i === 0 ? "bg-[#eff6ff]" : ""}`}
                  style={{ gridTemplateColumns: "116px 128px minmax(0,1fr) 140px 120px 120px 76px", gap: 8 }}
                >
                  <div className="n text-[12.5px] font-semibold text-[#0f172a]">{r.invoice_code}</div>
                  <div className="n text-[11.5px] text-[#64748b]">{formatDate(r.invoice_date)}</div>
                  <div className="truncate text-[12.5px] text-[#0f172a]">{r.customer?.store_name || "Khách lẻ"}</div>
                  <div className="truncate text-[12px] text-[#64748b]">{r.sales_user?.full_name || "—"}</div>
                  <div className="n text-right text-[12.5px] font-semibold text-[#16a34a]">
                    {formatCurrency(conTra)}
                  </div>
                  <div className="n text-right text-[12.5px] text-[#0f172a]">{formatCurrency(r.total)}</div>
                  <div className="text-right">
                    {conTra > 0 ? (
                      <button
                        type="button"
                        onClick={() => { onPick(r.id); onClose() }}
                        className="h-7 rounded-md bg-[#2563eb] px-3 text-[12px] font-semibold text-white"
                      >
                        Chọn
                      </button>
                    ) : (
                      <span className="text-[11.5px] text-[#94a3b8]">đã trả hết</span>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="flex h-11 shrink-0 items-center justify-between border-t border-[#e2e8f0] px-4">
