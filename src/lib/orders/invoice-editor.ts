...
/**
 * PHIẾU TRẢ ĐANG CHỜ SẼ VỠ NẾU LƯU TỜ HÓA ĐƠN NÀY.
 *
 * ⚠ CHỦ NHÀ BÁO 21/09/2026: sửa hóa đơn có hàng đổi/trả thì vấp
 * "REISSUE_BREAKS_RETURN: hóa đơn mới không còn bán "…" mà phiếu trả
 * đang chờ xử lý đòi trả. Huỷ phiếu trả trước, rồi sửa lại hóa đơn."
 *
 * ⚠ LUẬT Ở MÁY CHỦ ĐÚNG, CHỖ NÓI RA THÌ SAI. `reissue_invoice`
 * (mig 125) từ chối vì một lý do thật: khách chỉ trả được thứ đã THỰC
 * XUẤT, và `complete_return` lấy chính hóa đơn làm TRẦN số được trả.
 * Bỏ mất món khỏi hóa đơn mà vẫn giữ phiếu trả là để khách trả một món
 * chưa từng rời kho — nhập kho khống và trừ công nợ khống.
 *
 * Cái sai là người dùng chỉ biết điều đó SAU KHI đã sửa xong cả tờ hóa
 * đơn và bấm nút. Phép này tính đúng luật ấy ngay trên trình duyệt, để
 * màn hình nói ra từ lúc họ vừa bỏ dòng.
 *
 * ⚠ KHÔNG TỰ GỠ, KHÔNG TỰ XOÁ — ĐÃ CÂN NHẮC VÀ BỎ. Hai đường "tự động"
 * đều hỏng:
 *   · Gỡ `returns.invoice_id` về rỗng: `complete_return` (mig 127) khi
 *     đó rơi về đếm theo DÒNG ĐƠN, đúng cái trần lỏng mà trần theo hóa
 *     đơn sinh ra để thay — nhập kho khống bằng một đường khác.
 *   · Xoá dòng trả: hàng khách đã đưa lại là một sự thật vật lý; xoá
 *     dòng là mất cả hàng lẫn khoản trừ, không còn dấu vết.
 * Chọn giữ món trên hóa đơn hay sửa phiếu trả là một QUYẾT ĐỊNH CỦA
 * NGƯỜI DÙNG. Việc của màn hình là hỏi đúng lúc, và chỉ đường.
 *
...
export function returnsBrokenBy(
  ...
  const sold = new Set(
    rows.filter((r) => r.qty > 0 && !r.isExchange).map((r) => r.productId)
  )
  ...
  for (const l of pending) {
    ...
    const key = `${l.returnId}|${l.productId}`
    if (seen.has(key)) continue
    seen.add(key)
    out.push({ returnId: l.returnId, productName: l.productName })
  }
  return out
}
